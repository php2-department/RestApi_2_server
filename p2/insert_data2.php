...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	$stmt = $conn->prepare("INSERT INTO `members`(`id`,`name`,`email`) VALUES (?,?,?)");
	...
	// $sql = "INSERT INTO `member`(`id`,`name`,`email`) VALUES ('$id','$name','$email')";
	// if(!$conn->query($sql)){
	// 	$resultData = array('error' => true, 'message' => $conn->error);
	// 	echo json_encode($resultData);
	// 	return;
	// }
	// if($conn->query($sql)){	
	// 	$resultData = array('error' => false, 'message' => 'User Inserted', 'status'=>200);
	// 	echo json_encode($resultData);
	// 	return;
	// }
